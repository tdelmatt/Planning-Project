...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #   to see if a proposed PgNode_a has prenodes that are a 
        #   subset of the previous S level.  Once an
        #   action node is added, it MUST be connected to the S node instances 
        #   in the appropriate s_level set.
        
        #how do we connect nodes?
        
        #how do we get all of the actions?
        #we need a list of all possible actions for the problem
        
        
        #I would think you would want to set the actions 
        #themselves rather than pointers to them
        actionset = set()
        #for all actions
        for action in self.all_actions:
            
            #create planning graph action node
            newaction = PgNode_a(action)
            
            #my issue with the following- first of all not sure the set changes anything
            #wrt to the object
            
            #if the actions preconditions are contained in the previous s levels
            if newaction.prenodes.issubset(self.s_levels[level]):
                #then add the node to the proper action level of the planning graph
                actionset.add(newaction)
                
                #now connect child and parent nodes
                for parentnode in self.s_levels[level]:
                    if parentnode in newaction.prenodes:
                        newaction.parents.add(parentnode)
                        parentnode.children.add(newaction)
        self.a_levels.append(actionset)
        
        #currentaction = 
        #actionpossible = TRUE
        # for precondition in action.precond_pos:
        #    if precondition not in self.fs:#previous level literals
        #actionpossible = false
        #    if actionpossible == TRUE:
        #    actionlist.append(action)
        #return actionlist
        #instantiate action node in current level
        #connect action to preconditions
    ...
    def add_literal_level(self, level):
        ...
        #MY NOTE: PROBABLY WORTHWHILE TO THROW IN A CHECK IF SIZE OF LAST LEVEL 
        #OF LITERALS (OR ACTIONS FOR ACTION LEVEL) ISEQUAL TO SIZE OF ALL LITERALS
        #IN WHICH CASE WE CAN OMIT RUNNING PART OF THE REST OF THE ALGORITHM
        
        
        literalset = set()
        
        #literals would be in effects of actions from previous level
        ...
        for actionnode in self.a_levels[level-1]:
            for effnode in actionnode.effnodes:                
                newliteral = effnode
                literalset.add(newliteral)
                newliteral.parents.add(actionnode)
                actionnode.children.add(newliteral)
        self.s_levels.append(literalset)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for effectnode1 in node_a1.effnodes:
            for effectnode2 in node_a2.effnodes:
                if effectnode1.symbol  == effectnode2.symbol and effectnode1.is_pos  != effectnode2.is_pos:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effectnode1 in node_a1.effnodes:
            for prenode2 in node_a2.prenodes:
                if effectnode1.symbol  == prenode2.symbol and effectnode1.is_pos != prenode2.is_pos:
                    return True
        
        for prenode1 in node_a1.prenodes:
            for effectnode2 in node_a2.effnodes:
                if prenode1.symbol  == effectnode2.symbol and prenode1.is_pos != effectnode2.is_pos:
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #so why do we need parents here instead of preconditions
        #maybe this has something to do with the evaluation of is_mutex being 
        #already evaluated on parents and thus including the other mutex conditions 
        #from previous
        for parent1 in node_a1.parents:
            for parent2 in node_a2.parents:
                if parent2.is_mutex(parent1):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        #for all parent actions of both nodes
        for parent1 in node_s1.parents:
            for parent2 in node_s2.parents:
            #if there is a parent pair that is not mutually exclusive; return false
    ...
    def h_levelsum(self) -> int:
        ...
        #the cost is the level at which the goal first appears
        
        
        #for all literals in the goal
        #print(len(self.s_levels))
        #print(self.s_levels[0])
        goals = self.problem.goal
        #(for unittest there are two goals)
        for goalliteral in goals:
            #print(goalliteral)
            #while the literal has not been found
            literalisnotfound = True
            level = 0
            #print(level)
            while literalisnotfound and level < 10000000: 
                
                #for all literals in the current level
                for levelliteral in self.s_levels[level]:
                    if levelliteral.is_pos:
                        newliteral = levelliteral.symbol
                    else:
                        newliteral = '~{}'.format(levelliteral.symbol)
                    #print(level)
                    #print(levelliteral)
                    #if literal is found, add it to the sum
                    #turn on the flag so the while loop ends
                    #break out of the for loop
                    
                    
                    if newliteral == goalliteral:
                        #print(newliteral)
                        #print(goalliteral)
                        #print("goal found!")
                        level_sum += level
                        literalisnotfound = False
                        break
